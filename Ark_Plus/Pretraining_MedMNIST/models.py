...
def build_omni_model(args, num_classes_list):
    ...
    if args.pretrained_weights is not None:
        ...
        msg = model.load_state_dict(state_dict, strict=False)
        print('Loaded with msg: {}'.format(msg))
        # PATCH: safety check. With strict=False a wrong checkpoint would load silently
        # and the run would start from random weights. Only the attention masks
        # (recomputed buffers), the projector and the new heads may be missing.
        bad = [k for k in msg.missing_keys
               if not (k.endswith('attn_mask') or k.startswith('projector') or k.startswith('omni_heads'))]
        if bad:
            raise RuntimeError("Pretrained backbone did not load. Missing keys: {}".format(bad[:10]))
        print("Pretrained backbone check passed: all encoder weights loaded.")
